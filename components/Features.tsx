...
const Features: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-24">
      {features.map((f, i) => (
        <div key={i} className="glass p-8 rounded-[2rem] border-white/60 shadow-xl shadow-slate-100/50 hover:scale-[1.03] transition-all duration-300 group">
          <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-3xl mb-6 shadow-sm border border-slate-50 group-hover:rotate-6 transition-transform">
            {f.icon}
          </div>
